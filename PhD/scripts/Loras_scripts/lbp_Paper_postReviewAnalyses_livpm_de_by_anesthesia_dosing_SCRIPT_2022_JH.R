...
# setup
  ...
  suppressMessages(library(readxl))
  Sys.setenv(OMP_NUM_THREADS = 6) #limit parallel processing to 6 threads (a lightweight process that runs on a core)
...
#         iid              sid collection_date
#    <char>           <char>          <Date>
#1: PT-0060 LBPSEMA4BRAIN001      2015-06-26
#2: PT-0161 LBPSEMA4BRAIN002      2017-12-11
...
#    0.0   193.8   317.3   397.3   494.1  3316.0      91 
  ...
  # format --- replaced NAs with 0 hmmmmmmmmmmmmm
  ...
  # calculate the total amounts for each drug, bolus + infusion
  ...
  #there are 8 people where there are NAs (or 0s) for all 3 drugs
  summary(dt$MISS)
  #Mode   FALSE    TRUE 
  #logical     281       8 
  
  #keep only rows where drug data is present
  dt2 <- dt[MISS==FALSE]
...
# define drug dosing groups
  ...
  ## view distributions
  ##
  cor(dt2$dexmedetomidine, dt2$propofol) #Pearson's cor: [1] 0.1184813 -- Alex; but Lora is [1] 0.06835994 | 2 drugs are not likely to be given together
  ##changing NAs to 0 would skew this correlation
...
# bin living samples by dose | Stratify patients into dose groups (e.g., Q1 = lowest, Q4 = highest)

  ## propofol
  dt2 <- dt2[order(propofol)] # Sort by propofol dose (ascending)
  dt2[,propquartile := floor( 1 + 4 * (.I-1) / .N)] # Assign quartile based on row index
...
# update metadata -- treating anesthesia doses as a discrete variable vs continuous variable | each person is assigned into 1 of 4 quartiles for each of 3 meds
  met <- lbp$covariates
...
# contrasts
  p1Con <- getContrast(vob, form1, met, c(paste0("proDE","PM_PQ1"), paste0("proDE","LIV_PQ1"))) #comparing proDEPM_PQ1 and proDELIV_PQ1 | is there a diff in GE between these 2 groups, adjusted for covars and random effects
...
# correlate logFC values from DE results
  lCor <- c()
...
##this is the results of each dataset (ds1, ds2) and a reference dataset (livpm)
##seems like DE results dont change much according to anesthesia doses
