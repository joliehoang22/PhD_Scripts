...
rosmap_no_sva <-fread("/sc/arion/projects/mscic1/results/jolie/ROSMAP-MSBB/Rosmapv30/rosmap_no_sva_1.11.txt", data.table = FALSE) #19476     7 | this code was used in DEA_NB.sh - FINAL VERSION
...
###############################################################################
####### new plots with updated color and correlation values in the tiles
plot <- ggplot(heatmap_df, aes(x = MSBB_Label, y = ROSMAP_Label, fill = Correlation)) +
  ...
  scale_fill_gradient2(low = "#fff5d5", mid = "#ef8a26", high = "#e0413f") +
  coord_fixed() +
  theme_minimal(base_size = 14) +  # Increases overall theme text size
  theme(
  ...
  labs(
    ...
    x = "MSBB Surrogate Variables",
    y = "ROSMAP Surrogate Variables"
  )
...
ps1 <- ggplot(heatmap_df_subset1, aes(x = MSBB_Label, y = ROSMAP_Label, fill = Correlation)) +
  ...
  scale_fill_gradient2(low = "#fff5d5", high = "#e0413f") +
  ...
  labs(
    title = "Spearman Correlation Heatmap (SVs = 0 to 10)",
    x = "MSBB Surrogate Variables",
    y = "ROSMAP Surrogate Variables"
  )

ggsave("/hpc/users/hoangd02/www/plots/rosmap_msbb_corr_0_to_10_heatmap_with_overlapping_genes_and_cor_values_20250620.pdf", plot = ps1, width = 8, height = 8)
...
ps1 <- ggplot(heatmap_df_subset1, aes(x = hbcc_Label, y = mssm_Label, fill = Correlation)) +
  ...
  scale_fill_gradient2(mid = "#FFFFC5", high = "#e0413f") +
  #scale_fill_gradient2(low = "#a0f600", high = "#d633a6") + #this is equivalent to most pink and low = white
...
ggsave("/hpc/users/hoangd02/www/plots/bp_mssm_hbcc_corr_0_to_10_heatmap_with_overlapping_genes_and_cor_values_test_20250620.pdf", plot = ps1, width = 8, height = 8)
...
allResults <- allResults %>%
  ...
  mutate(
    ...
    facet_label = case_when(
      data == "ROSMAP" ~ "ROSMAP (Wilcoxon test: p = 0.015)",
      data == "MSBB" ~ "MSBB (Wilcoxon test: p = 6.87e-07)"
    )
  )

# Step 2: Create custom y-axis breaks
breaks_rosmap <- 10^-(0:3 * 100)  # 1, 1e-100, 1e-200, 1e-300
breaks_msbb   <- 10^-(0:3 * 10)   # 1, 1e-10, 1e-20, 1e-30
