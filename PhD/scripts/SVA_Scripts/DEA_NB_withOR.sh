...
## OR interpretation
## odds: For each external dataset, build this 2×2 table:

#								In external list 					Not in external list
# My DEG 							a 													b
# My non-DEG					c 													d

## Odds ratio: (a/b) / (c/d) = ad / bc

## OR > 1 means that my DEGs are enriched in the external list (more overlap than expected)
## OR = 1 means no enrichment
## OR < 1 means depletion

## take log(OR) to centers it around 0 (OR = 1 --> logOR = 0) and deviations above and below are equally spaced --> symmetry which helps with Wilcoxon signed-rank test, which assume roughly symmetric differences around 0.

#Helps with interpretation, comparing ORs = nonlinear; comparing logORs means comparing simple differences; On the log scale, difference = log(4) − log(2) = log(2) → “2× higher enrichment.”

## My DEG = with and without SVA, respectively
...
wilcox.test(
	allResults$pval[allResults$adjustment=="noSVA" & allResults$data=="MSBB"],
	allResults$pval[allResults$adjustment=="SVA_be" & allResults$data=="MSBB"],
	alternative="less"
	)

wilcox.test(
	allResults$pval[allResults$adjustment=="noSVA" & allResults$data=="ROSMAP"],
	allResults$pval[allResults$adjustment=="SVA_be" & allResults$data=="ROSMAP"],
	alternative="less"
	)

############## WILCOXON ON OR ##############
wilcox.test(
  log(allResults$OR[allResults$adjustment == "noSVA" & allResults$data == "MSBB"]),
  log(allResults$OR[allResults$adjustment == "SVA_be" & allResults$data == "MSBB"]),
  alternative = "greater",   
  paired = TRUE
) 
#paired: V = 120, p-value = 0.07076
#non-paired: W = 197, p-value = 0.8675

wilcox.test(
  log(allResults$OR[allResults$adjustment == "noSVA" & allResults$data == "ROSMAP"]),
  log(allResults$OR[allResults$adjustment == "SVA_be" & allResults$data == "ROSMAP"]),
  alternative = "greater",   
  #paired = TRUE
) 
#paired: V = 125, p-value = 0.04488
#non-paired: W = 207, p-value = 0.0805
