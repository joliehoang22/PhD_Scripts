...
compute_rb_for_tissue <- function(brainmeta, tissue_df, p_thresh = 5e-8, theta = 0) {
  ...
  #    (assuming BrainMeta p-value column is named 'p')
...
## -------------------------
## 4. nsv handling via command line args
## -------------------------
args <- commandArgs(trailingOnly = TRUE)

if (length(args) >= 2) {
  nsv_start <- as.integer(args[1])
  nsv_end   <- as.integer(args[2])
} else {
...
out_path <- sprintf(
  "/sc/arion/projects/mscic1/results/jolie/LBP/eqtl/rb_brainmeta_liv_pm_by_nsv%dto%d_20251205.csv",
  nsv_start, nsv_end
)
