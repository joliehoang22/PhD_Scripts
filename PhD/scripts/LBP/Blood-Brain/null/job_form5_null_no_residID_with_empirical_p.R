##### code derived from form5_null_with_empirical_p.R
#blood_brain_null_form0.R | OG_null_with_empirical_p.R (this is not as comprehensive)| LBP_blood_brain_QC_20250603.rmd
rm(list=ls())
library(data.table)
library(ggplot2)
library(readxl)
...
# blood_id <- "job_blood_form5_with_ID" 
...
blood_id <- "job_blood_form5_no_residID" 
...
bsub -q premium -P acc_mscic1 -n 30 -W 144:00 -R rusage[mem=4000] -R span[hosts=1] -o %J.stdout -eo %J.stderr Rscript ${path}job_form5_null_with_empirical_p.R
bsub -q premium -P acc_mscic1 -n 30 -W 144:00 -R rusage[mem=4000] -R span[hosts=1] -o %J.stdout -eo %J.stderr Rscript ${path}job_form5_null_no_residID_with_empirical_p.R

# Job <203382301> is submitted to queue <premium>.
# Job <203382302> is submitted to queue <premium>.