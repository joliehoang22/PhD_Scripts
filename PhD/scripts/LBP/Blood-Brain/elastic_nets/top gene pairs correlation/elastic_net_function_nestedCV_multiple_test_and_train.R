## Improved version of elastic_net_function_data_leakage.R and elastic_net_funtion_nestedCV_single_test_and_train.R
## Feature selection on training data only | CV with multiple train/test split approach

# Improved nested CV function for predicting brain gene expression from blood
# Prevents data leakage with proper nested cross-validation and hyperparameter tuning

## For more info, read: https://cran.r-project.org/web/packages/nestedcv/vignettes/nestedcv.html
...
#   X_all = blood_expression_matrix,  # samples x blood_genes
#   Y_all = brain_expression_matrix,  # samples x brain_genes  
#   brain_genes = colnames(brain_expression_matrix)[1:100],  # test subset first
#   n_outer_folds = 5,
#   n_inner_folds = 5,
#   top_percent = 0.05,
#   alpha_values = c(0, 0.25, 0.5, 0.75, 1),
