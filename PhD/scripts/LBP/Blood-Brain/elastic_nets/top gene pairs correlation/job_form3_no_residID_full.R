#code derived from form3_no_residID_full.R
library(data.table)
library(ggplot2)
library(readxl)
library(biomaRt)
...
bsub -q premium -P acc_mscic1 -n 30 -W 144:00 -R rusage[mem=4000] -R span[hosts=1] -o %J.stdout -eo %J.stderr Rscript ${path}job_form3_no_residID_base.R
bsub -q premium -P acc_mscic1 -n 30 -W 144:00 -R rusage[mem=4000] -R span[hosts=1] -o %J.stdout -eo %J.stderr Rscript ${path}job_form3_no_residID_full.R
bsub -q premium -P acc_mscic1 -n 30 -W 144:00 -R rusage[mem=4000] -R span[hosts=1] -o %J.stdout -eo %J.stderr Rscript ${path}job_form3_with_residID_base.R
bsub -q premium -P acc_mscic1 -n 30 -W 144:00 -R rusage[mem=4000] -R span[hosts=1] -o %J.stdout -eo %J.stderr Rscript ${path}job_form3_with_residID_full.R

#bjobs in the order above
#Job <198848492> is submitted to queue <premium>.
#Job <198848493> is submitted to queue <premium>.
#Job <198848495> is submitted to queue <premium>.
#Job <198848496> is submitted to queue <premium>.
...
write.table(results, file = "/sc/arion/projects/mscic1/results/jolie/LBP/blood-brain/qc2/top_brain_full_genes_elastic_net_blood_form3_no_indivdualID_nestedCV_multiple_job.txt", 
