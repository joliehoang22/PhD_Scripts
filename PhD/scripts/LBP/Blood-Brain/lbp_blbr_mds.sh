...
#this is the function that takes long 
...
mds <- readRDS("/sc/arion/projects/mscic1/results/jolie/LBP/blood-brain/mds_gtex_lbp_rosmap_msbb.RDS")
...
g1=ggplot(mds2[shuffled,],aes(x=MDS1, y=MDS2, color=SMTS, label = SMTS)) + geom_text(size = 3) + 
scale_color_discrete(name = "Tissue") + 
theme_bw()

#g2 is specific brain regions
...
g1 <- ggplot(mds2[shuffled, ], aes(x = MDS1, y = MDS2, color = SMTS, label = SMTS)) +
  geom_text(size = 3, fontface = "bold") +  # match large, bold text for poster
  scale_color_discrete(name = "Tissue") +
  labs(
    x = "MDS1",
    y = "MDS2"
...
ggsave("/hpc/users/hoangd02/www/plots/lbp_mds_wcpg.pdf", plot = g1, width = 6, height=6) 
